...
if __name__ == "__main__":
    ...
    real_tensor = torch.ones(3, 4, 4, dtype=torch.float).cuda()
    # start train
    for epoch in range(opt.start_epoch, opt.non_decay+opt.decay+1):
        ...
        for iteration, data in enumerate(training_data_loader):
            ...
            loss_sync = criterionMSE(torch.sigmoid(sync_score).view(-1, 4, 4), real_tensor)
            # combine all losses
            ...
            print(
                "===> Epoch[{}]({}/{}):  Loss_DI: {:.4f} Loss_GI: {:.4f} Loss_DV: {:.4f} Loss_GV: {:.4f} Loss_perception: {:.4f} compute_Loss_sync: {:.6f}  real_loss_sync: {:.6f}  lr_g = {:.7f} ".format(
                    epoch, iteration, len(training_data_loader), float(loss_dI), float(loss_g_dI),float(loss_dV), float(loss_g_dV), float(loss_g_perception), float(loss_sync), float(loss_sync * sycnet_loss_weight),
...
#     # set label of syncnet perception loss
#     # real_tensor = torch.tensor(1.0).cuda()
#     real_tensor = torch.ones(1, 4, 4, dtype=torch.float).cuda()
#     # start train
#     for epoch in range(opt.start_epoch, opt.non_decay+opt.decay+1):
#         net_g.train()
#         for iteration, data in enumerate(training_data_loader):
...
#             fake_out = net_g(source_clip_mask,reference_clip,deep_speech_clip)
#             print(source_clip.shape, fake_out.shape)
...
#             # if loss_g_perception.item() <= 1.8:
#             #     opt.lamb_syncnet_perception = 0.01
#             # else:
#             #     opt.lamb_syncnet_perception = 0.0
...
#             train_data.radius_1_4:train_data.radius_1_4 + train_data.mouth_region_size]
#             sync_score = net_lipsync(fake_out_clip_mouth, deep_speech_full)
#             print(sync_score.shape, real_tensor.shape)
