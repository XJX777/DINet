...
from utils.data_processing_new_ import load_landmark_openface,compute_crop_radius
...
def crop_face_according_openfaceLM(openface_landmark_dir,video_frame_dir,res_crop_face_dir,clip_length):
    ...
    landmark_openface_path_list = glob.glob(os.path.join(openface_landmark_dir, '*.json'))
    for landmark_openface_path in landmark_openface_path_list:
        video_name = os.path.basename(landmark_openface_path).replace('.mp4.json', '')
...
#             # 遍历每个关键点并在图像上进行标注
#             for i, (x, y) in enumerate(landmark_openface_data[0, :, :]):
#                 # 绘制关键点的圆圈
#                 cv2.circle(first_image, (x, y), 1, (0, 0, 255), -1)  # 红色圆圈

#                 # 在关键点旁边添加点位序号
#                 # if i == 54 or i == 33 or i == 29 or i == 48:
#                 if i == 45 or i == 49 or i == 84 or i == 90:
#                     cv2.putText(first_image, str(i), (x+1, y-1), cv2.FONT_HERSHEY_SIMPLEX, 0.3, (0, 0, 255), 1)
                
#             cv2.imwrite('see.jpg', first_image)
#             pdb.set_trace()
            ...
            for frame_index in range(end_frame_index[i]- clip_length,end_frame_index[i]):
                ...
                #                     :].copy()
                crop_face_data = source_frame_data[
                                    frame_landmark[45, 1] - radius_clip:frame_landmark[
                                                                            45, 1] + radius_clip * 2 + radius_clip_1_4,
                                    frame_landmark[49, 0] - radius_clip - radius_clip_1_4:frame_landmark[
                                                                                              49, 0] + radius_clip + radius_clip_1_4,
                                    :].copy()
                
                # # 29 to 45. # 54 to 90.  48 to 84.  33 to 49. 
